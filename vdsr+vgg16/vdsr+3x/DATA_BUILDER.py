...
class DATA:    
    ...
    def __init__(self , folder = './' , patch_size = 96 ):
            self.folder = folder
    ...
    def load_data(self , folder=''):
            self.training_patches_Y  = np.load('./training_patches_Y.npy') 
            if self.patch_size == 64:
                self.training_patches_2x = np.load('./training_patches_2x.npy') 
            elif self.patch_size == 96:
                self.training_patches_3x = np.load('./training_patches_3x.npy') 
            elif self.patch_size == 128:
                self.training_patches_4x = np.load('./training_patches_4x.npy') 
            elif self.patch_size == 256:
                self.training_patches_8x = np.load('./training_patches_8x.npy') 
            print(folder , " extracted in !!")
    ...
    def construct_list(self ):
            ...
            for i in range(len(train_data_list)):
                ...
                if self.patch_size == 64:
                    ...
                    img=cv2.resize(img , (int(img.shape[1]*2),int(img.shape[0]*2)) ,interpolation=cv2.INTER_LINEAR)
                    self.training_patches_2x += self.patchify(img , scale=1)[0]
                elif self.patch_size == 96:
                    img=cv2.resize(img , (int(img.shape[1]/3),int(img.shape[0]/3)) ,interpolation=cv2.INTER_LINEAR)
                    img=cv2.resize(img , (int(img.shape[1]*3),int(img.shape[0]*3)) ,interpolation=cv2.INTER_LINEAR)
                    self.training_patches_3x += self.patchify(img , scale=1)[0]
                elif self.patch_size == 128:
                    img=cv2.resize(img , (int(img.shape[1]/4),int(img.shape[0]/4)) ,interpolation=cv2.INTER_LINEAR)
                    img=cv2.resize(img , (int(img.shape[1]*4),int(img.shape[0]*4)) ,interpolation=cv2.INTER_LINEAR)
                    self.training_patches_4x += self.patchify(img , scale=1)[0]
                elif self.patch_size == 256:
                    img=cv2.resize(img , (int(img.shape[1]/8),int(img.shape[0]/8)) ,interpolation=cv2.INTER_LINEAR)
                    img=cv2.resize(img , (int(img.shape[1]*8),int(img.shape[0]*8)) ,interpolation=cv2.INTER_LINEAR)
                    self.training_patches_8x += self.patchify(img , scale=1)[0]
...
if __name__ == "__main__":
    import time
    s = time.time()
    d = DATA(folder='./291') ## Call for constructing the Object. Give the root directory where it can find the
    # images. It will walk and find everything
    d.construct_list() # only when you need to create everything a new
    print('amount of data:' , d.training_patches_Y.shape,d.training_patches_3x.shape)
